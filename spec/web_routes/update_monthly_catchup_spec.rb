describe 'Update monthly_catchup' do
  ...
  before do
    stub_instances(UI::UseCase::UpdateMonthlyCatchup, update_monthly_catchup_spy)
  ...
  context 'calls the update monthly_catchup usecase' do
    example 1 do
      post '/monthly-catch-up/update', {
        project_id: 1,
        monthly_catchup_id: 2,
        monthly_catchup_data: {
          date: '09/09/1989'
        }
      }.to_json, 'HTTP_API_KEY' => api_key
      expect(update_monthly_catchup_spy).to have_received(:execute).with(project_id: 1, monthly_catchup_id: 2, monthly_catchup_data: { date: '09/09/1989' })
    end

    example 2 do
      post '/monthly-catch-up/update', {
        project_id: 5,
        monthly_catchup_id: 9,
        monthly_catchup_data: {
          date: '02/06/2005'
        }
      }.to_json, 'HTTP_API_KEY' => api_key
      expect(update_monthly_catchup_spy).to have_received(:execute).with(project_id: 5, monthly_catchup_id: 9, monthly_catchup_data: { date: '02/06/2005' })
    end
  end
  ...
  context 'API Key' do
    context 'example 1' do
      before do
        post '/monthly-catch-up/update',
          { project_id: 99, monthly_catchup_id: 9, monthly_catchup_data: { date: '11/02/2018'} }.to_json, 'HTTP_API_KEY' => api_key
      end
      ...
      context 'is not in header' do
        it 'responds with a 400' do
          post '/monthly-catch-up/update',
            { project_id: 99, monthly_catchup_id: 9, monthly_catchup_data:{ date: '11/02/2018'} }.to_json
    ...
    context 'example 2' do
      before do
        post '/monthly-catch-up/update',
          { project_id: 42, monthly_catchup_id: 1, monthly_catchup_data:{ date: '05/17/2018'} }.to_json, 'HTTP_API_KEY' => api_key
      end
      ...
      context 'is not in header' do
        it 'responds with a 400' do
          post '/monthly-catch-up/update',
            { project_id: 52, monthly_catchup_id: 2, monthly_catchup_data:{ date: '10/09/2018'} }.to_json
            expect(last_response.status).to eq(400)
        end
      end
    end
