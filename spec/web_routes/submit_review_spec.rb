fdescribe 'submit review' do
  ...
  let(:submit_review_spy) { spy }

  before do
    stub_instances(HomesEngland::UseCase::SubmitReview, submit_review_spy)
  ...
  context 'calls the submit use case' do
    example 1 do
      post '/review/submit', { project_id: 3, review_id: 2 }.to_json, 'HTTP_API_KEY' => api_key
      expect(submit_review_spy).to have_received(:execute).with(review_id: 2)
    end

    example 2 do
      post '/review/submit', { project_id: 7, review_id: 9 }.to_json, 'HTTP_API_KEY' => api_key
      expect(submit_review_spy).to have_received(:execute).with(review_id: 9)
    end
  end
  ...
  context 'API Key' do
    context 'example 1' do
      before do
        post '/review/submit',
          { project_id: 99, review_id: 9 }.to_json, 'HTTP_API_KEY' => api_key
      end
      ...
      context 'is not in header' do
        it 'responds with a 400' do
          post '/review/submit',
            { project_id: 99, review_id: 9 }.to_json
    ...
    context 'example 2' do
      before do
        post '/review/submit',
          { project_id: 42, review_id: 1 }.to_json, 'HTTP_API_KEY' => api_key
      end
      ...
      context 'is not in header' do
        it 'responds with a 400' do
          post '/review/submit',
            { project_id: 52, review_id: 2 }.to_json
            expect(last_response.status).to eq(400)
        end
      end
    end
