...
fdescribe 'Migration 21' do
  ...
  context 'Example 1' do
    ...
    it 'Migrates the version number to the baselines table' do
      baseline_version = migrated_baseline_data[:version]

      expect(baseline_version).to eq(1)
    end
    ...
    it 'Removes the baseline data information from the project table' do
      project_data = migrated_project_data[:data]
      project_status = migrated_project_data[:status]
      project_version = migrated_project_data[:version]

      expect(project_data).to be_nil
      expect(project_status).to be_nil
      expect(project_version).to be_nil
    end
  end
