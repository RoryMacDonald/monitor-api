describe UI::UseCase::ConvertCoreClaim do
  context 'example 1' do
    let(:convert_core_hif_claim_spy) { spy(execute: { data: 'hif data' }) }
    let(:convert_core_ac_claim_spy) { spy(execute: { data: 'ac data' }) }
    let(:convert_core_ff_claim_spy) { spy(execute: { data: 'ff data' }) }

    let(:usecase) do
      described_class.new(
        convert_core_ac_claim: convert_core_ac_claim_spy,
        convert_core_hif_claim: convert_core_hif_claim_spy,
        convert_core_ff_claim: convert_core_ff_claim_spy
      )
    end
    ...
    context 'hif' do
      it 'calls the convert use case' do
        ...
        expect(convert_core_hif_claim_spy).to have_received(:execute).with(
    ...
    context 'ac' do
      it 'calls the convert use case' do
        ...
        expect(convert_core_ac_claim_spy).to have_received(:execute).with(
          claim_data: { some_data: 'data value' }
        )
      end
    ...
    context 'ff' do
      it 'calls the convert use case' do
        ...
        expect(convert_core_ff_claim_spy).to have_received(:execute).with(
  ...
  context 'example 2' do
    let(:convert_core_hif_claim_spy) { spy(execute: { data: 'converted return data' }) }
    let(:convert_core_ac_claim_spy) { spy(execute: { data: 'ac converted return data' }) }
    let(:convert_core_ff_claim_spy) { spy(execute: { data: 'ff converted return data' }) }
